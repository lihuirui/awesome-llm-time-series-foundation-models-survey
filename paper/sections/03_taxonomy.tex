...
\begin{itemize}[leftmargin=*]
    \item \emph{Native Time Series Foundation Models (Native TSFMs)}: Models built from scratch and trained directly on large-scale collections of empirical or synthetic time-series data. Weights and latent dimensions are optimized specifically for temporal signals without textual preconceptions.
    \item \emph{Repurposed Large Language Models (LLM4TS)}: Models that adopt pre-existing weights from language or vision transformers (e.g., Llama-2/3, GPT-2), bridging the continuous temporal modality into text embeddings via reprogramming linear layers, prompting prefixes, or cross-attention adapters.
    \item \emph{Evaluation Frameworks and Benchmark Audits}: Standardized benchmarking suites and diagnostic harnesses that audit generalization, benchmark contamination, and probabilistic calibration.
\end{itemize}

\subsubsection{Dimension 2: Architectural Backbone}
\begin{itemize}[leftmargin=*]
    \item \emph{Decoder-Only Autoregressive Transformers}: Optimized for sequential next-token or next-patch generation (e.g., TimesFM~\cite{das2024timesfm}, Timer~\cite{liu2024timer}, Chronos~\cite{ansari2024chronos}, Sundial~\cite{thuml2025sundial}).
    \item \emph{Masked Encoder Transformers}: Optimized for bidirectional temporal representations via masked autoencoding (e.g., MOMENT~\cite{goswami2024moment}, PatchTST~\cite{nie2023patchtst}).
    \item \emph{Encoder-Decoder Transformers}: Decoupling context representation from horizon generation (e.g., MOIRAI~\cite{woo2024moirai}, TTM~\cite{ekambaram2024ttm}).
...
\begin{itemize}[leftmargin=*]
    \item \emph{Uniform Subseries Patching}: Continuous linear projection of non-overlapping or strided time-step windows into dense embedding space.
    \item \emph{Scalar Discretization / Binning}: Quantizing scaled continuous observations into finite categorical tokens, allowing standard NLP cross-entropy training.
    \item \emph{Decimal String Tokenization}: Casting continuous numbers into ASCII strings (e.g., ``12.4''), processed directly by off-the-shelf text tokenizers (Byte-Pair Encoding).
    \item \emph{Multi-Resolution and Adaptive Patching}: Dynamically adjusting patch lengths across frequencies to resolve multi-scale seasonalities.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \emph{Continuous Flow Matching and Diffusion}: Formulating generation as an equivariant continuous-time velocity field (FlowState~\cite{king2025flowstate}).
\end{itemize}
